...
- [x] **Budget**: No AI/LLM API calls in this feature — same as Feature 003. The only external calls are to the Instagram Graph API (free for publishing), the Google Drive API (already used, free tier) and the Telegram Bot API (free), so this feature adds no spend to govern. FR-015 has been **removed** from the spec for that reason; see "Decision: FR-015 removed" in `spec.md`. *(Corrected 2026-09-25: this line previously said FR-015 was satisfied by "the pipeline's existing shared budget-guard check". No such check exists in the repository. Budget control for model spend is tracked separately.)*
