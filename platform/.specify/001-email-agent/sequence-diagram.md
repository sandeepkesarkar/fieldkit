...
sequenceDiagram
    ...
    note over OpenClaw,Script: Trigger: system cron every 5 min<br/>OR admin sends /check_email
    ...
    note over Script,state: PHASE 1 — resolve fk-received label ID once per run
    ...
    note over Script,state: PHASE 2 — stale check: alert admin if any acks undelivered >15 min
    ...
    opt Stale entries found
        ...
        alt Alert email sent successfully
        ...
        else Alert email failed (gws error)
            note over Script: log STALE_ALERT_SEND_FAILED<br/>entries remain in pending for next cycle
        end
    end

    note over Script,Gmail: PHASE 3 — list unread emails not yet labelled
    ...
    opt Gmail returns nextPageToken (inbox has >100 unread unlabeled messages)
        Script->>Telegram: openclaw message send "⚠ Gmail inbox overflow…"
        Telegram-->>Admin: overflow warning
        note over Script: only first 100 messages processed this cycle
    end
    ...
    alt No unread messages
        ...
        loop For each message
            ...
            alt Sender NOT in ADMIN_ALLOWLIST
                note over Script: sanitize From/Subject:<br/>truncate to 200 chars, strip \n \r \t
                ...
                alt Message ID already in processed map (crash recovery)
                    ...
                    state->>state: increment last_ref_id<br/>record gmail_message_id → ref_id<br/>write state.json<br/>(evict oldest entries if map > 10,000)
...
| Telegram unreachable | `_telegram()` logs non-zero exit at WARNING; message stays in `pending.json`; stale alert email sent after 15 min |
| Stale alert email fails | Log `STALE_ALERT_SEND_FAILED` at ERROR; stale entries remain in `pending.json` for the next cycle — no dequeue |
...
| State error mid-loop (corrupt state.json) | Telegram error sent, `log_cycle(processed, rejected)` written with partial counts, script exits 1 |
| Inbox overflow (>100 unread unlabeled) | Telegram warning sent; first 100 processed; remainder handled next cycle |
